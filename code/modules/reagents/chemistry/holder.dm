		...
		if(C && R)
			if(C.reagent_check(R) != TRUE)
				...
				if(can_overdose)
					...
					if((prob(R.addiction_threshold) || (prob(0.05) && R.addiction_threshold)) && !is_addicted_to)  ///{T.WINER} - В теории теперь у нас зависит все не от количества вещества, а от процента в тик
