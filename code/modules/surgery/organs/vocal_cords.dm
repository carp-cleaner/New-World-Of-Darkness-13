...
/proc/voice_of_ghost_god(message, mob/user, base_multiplier = 1, message_admins = TRUE)
	var/cooldown = 0
	...
	for(var/mob/dead/D in range(8, user))
		if(D)
			to_chat(D, "<span class='phobia'>[uppertext(message)]</span>")
	...
	for(var/mob/living/L in get_hearers_in_view(8, user))
		if(L.can_hear() && L.stat != DEAD)
			var/mob/living/carbon/human/H = L
			to_chat(L, "<span class='phobia'>[uppertext(message)]</span>")
			if(istype(H.ears, /obj/item/clothing/ears/earmuffs))
				continue
			if(L.resistant_to_disciplines)
				continue
			H.cure_trauma_type(/datum/brain_trauma/hypnosis/dominate, TRAUMA_RESILIENCE_MAGIC)
			H.gain_trauma(new /datum/brain_trauma/hypnosis/dominate(uppertext(message)), TRAUMA_RESILIENCE_MAGIC)
			spawn(30 SECONDS)
				if(H)
					H.cure_trauma_type(/datum/brain_trauma/hypnosis/dominate, TRAUMA_RESILIENCE_MAGIC)
			listeners += L
	...
	*/
	...
	var/static/regex/stun_words = regex("stop|wait|stand still|hold on|halt|cease|стой|остановись|остановитесь|стоять|стойте|стоп")
	var/static/regex/knockdown_words = regex("drop|fall|trip|knockdown|упадите|падать|ложитесь|на пол|лицом вниз|к земле|на колени|упади")
	var/static/regex/sleep_words = regex("sleep|slumber|rest|спать|отдых|спите|спи|сон|засни|усни|засните|закройте глаза|закрой глаза")
	var/static/regex/vomit_words = regex("vomit|throw up|sick|рыгать|рвота|проблюйся|проблеваться|рыгай|рыгайте|освободи желудок|освободите желудок")
	var/static/regex/silence_words = regex("shut up|silence|be silent|ssh|quiet|hush|тише|заткнись|заткнитесь|замолчи|замолчите|молчать|тишина|шшш")
	var/static/regex/hallucinate_words = regex("see the truth|hallucinate|узри правду|узрите правду")
	var/static/regex/wakeup_words = regex("wake up|awaken|очнись|очнитесь|проснись|проснитесь|открой глаза|откройте глаза")
	var/static/regex/heal_words = regex("live|heal|survive|mend|life|heroes never die|живи|живите|лечись|лечитесь|выздоравливай|выздоравливайте|герои не умирают")
	var/static/regex/hurt_words = regex("die|suffer|hurt|pain|death|умри|сдохни|умрите|сдохните|боль|страдай|страдайте")
	var/static/regex/bleed_words = regex("bleed|there will be blood|истекай кровью|истеки кровью|истекайте кровью|кровоточи|кровоточите|кровь")
	var/static/regex/burn_words = regex("burn|ignite|гори|горите|полыхай|полыхайте|пламя")
	var/static/regex/hot_words = regex("heat|hot|hell|горячо|согрейся|согрейтесь|подогрев")
	var/static/regex/cold_words = regex("cold|cool down|chill|freeze|остынь|остыньте|замерзни|замёрзни|замёрзните|замерзните|охладитесь|охладись|холод")
	var/static/regex/repulse_words = regex("shoo|go away|leave me alone|begone|flee|fus ro dah|get away|repulse|прочь|уходи|уходите|убирайся|убирайтесь|изыди|изыдите|брысь")
	var/static/regex/attract_words = regex("come here|come to me|get over here|attract|сюда|подойди|подойдите|ко мне|приблизься|приблизтесь")
	var/static/regex/whoareyou_words = regex("who are you|say your name|state your name|identify|кто ты|кто вы|назовись|назовитесь|назови имя")
	var/static/regex/saymyname_words = regex("say my name|who am i|whoami|кто я|мое имя|моё имя|меня зовут")
	var/static/regex/knockknock_words = regex("knock knock|тук тук|тук-тук|туц туц|туц-туц")
	//var/static/regex/statelaws_words = regex("state laws|state your laws")
	var/static/regex/move_words = regex("move|walk|иди|идите")
	var/static/regex/left_words = regex("left|west|port|налево|запад")
	var/static/regex/right_words = regex("right|east|starboard|направо|восток")
	var/static/regex/up_words = regex("up|north|fore|вверх|север")
	var/static/regex/down_words = regex("down|south|aft|вниз|юг")
	var/static/regex/walk_words = regex("slow down|шагом|медленнее")
	var/static/regex/run_words = regex("run|бегом|быстрее")
	var/static/regex/helpintent_words = regex("help|hug|помоги|помогите")
	var/static/regex/disarmintent_words = regex("disarm|толкни|толкните")
	var/static/regex/grabintent_words = regex("grab|схвати|схватите")
	var/static/regex/harmintent_words = regex("harm|fight|punch|ударь|ударьте")
	var/static/regex/throwmode_words = regex("throw|catch|кидай|лови|кидайте|ловите")
	var/static/regex/flip_words = regex("flip|rotate|revolve|roll|somersault|крутись|крутитесь|вокруг")
	var/static/regex/speak_words = regex("speak|say something|говори|говорите")
	var/static/regex/getup_words = regex("get up|подъем|подъём|поднимись|поднимитесь")
	var/static/regex/sit_words = regex("sit|садись|садитесь|сидеть")
	var/static/regex/stand_words = regex("stand|встань|встаньте|встать")
	var/static/regex/dance_words = regex("dance|танцуй|танцуйте")
	var/static/regex/jump_words = regex("jump|прыгай|прыгайте")
	var/static/regex/salute_words = regex("salute|отдать честь")
	var/static/regex/deathgasp_words = regex("play dead|притворись мёртвым|притворись мертвым|притворитесь мёртвым|притворитесь мертвым")
	var/static/regex/clap_words = regex("clap|applaud|аплодисменты|овации|похлопай|похлопайте")
	var/static/regex/honk_words = regex("ho+nk") //hooooooonk
	var/static/regex/multispin_words = regex("like a record baby|right round")
